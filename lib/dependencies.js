...
class DependendenciesManager {
  ...
  _setBowerCommandRoot() {
    ...
    .then(hasBower => {
      if (!hasBower) {
        ...
        .then(() => fs.pathExists(path.join(this.workingDir, localCommand)))
        .then(exists => {
          if (!exists) {
            throw new Error('Unable to install bower locally.');
          }
          this.logger.info('Bower has been installed locally...');
          return true;
        });
      }
      return false;
    ...
    .then(isLocal => {
      ...
      this.logger.info('Setting bower command to: ', this.commandRoot);
    });
  }
  ...
  isBowerInstalled() {
    ...
    if (isWin) {
      ...
      whereCommand = 'whereis';
    }
    return this.exec(whereCommand + ' bower')
    .then(out => {
      out = out || '';
      if (isWin) {
    ...
    return new Promise((resolve, reject) => {
      ...
      exec(cmd, opts, (err, stdout, stderr) => {
        if (err) {
          ...
          reject(new Error('  Unable to execute previous command with error: ' +
